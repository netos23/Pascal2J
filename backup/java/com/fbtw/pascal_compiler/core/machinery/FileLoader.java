...
import com.fbtw.pascal_compiler.core.data_types.memory_core.MemoryCell;
...
public class FileLoader {
    public static final String[] KEY_WORDS = {";", ":", "program", "var", "begin", "end", ".", "procedure",
            ...
            "longinteger", "real", "double", "boolean", "char", "array", "list", "stack", "map", "set", "queue", "deque", "[", "]", "=", "const", "'", "/", "{", "}"};
    ...
    public FileLoader(String fileName) throws Exception {
        ...
        while (in.hasNext()) {
            ...
            if (endIndex > beginIndex && !isCommentary) {
    ...
    public void loadProgram() throws Exception {
        ...
        for (int i = 0; i < strings.length; i++) {
            ...
            if (codeForAnalis.contains(" " + KEY_WORDS[4] + " ")
                    || (codeForAnalis.contains(KEY_WORDS[4] + " ")
                    && codeForAnalis.indexOf(KEY_WORDS[4] + " ") == 0)) { //main program
                checkStartOfLine(codeForAnalis, KEY_WORDS[4] + " ");
                for (; i < strings.length; i++) {
                    mainProg.add(strings[i]);
                }
            } else if (codeForAnalis.contains(" " + KEY_WORDS[2] + " ")
                    || (codeForAnalis.contains(KEY_WORDS[2] + " ")
                    && codeForAnalis.indexOf(KEY_WORDS[2] + " ") == 0)) { // key word program
                checkStartOfLine(codeForAnalis, KEY_WORDS[2] + " ");
                if (progName.equals("")) {
            ...
            } else if (codeForAnalis.contains(" " + KEY_WORDS[3] + " ")
                    || (codeForAnalis.contains(KEY_WORDS[3] + " ")
                    && codeForAnalis.indexOf(KEY_WORDS[3] + " ") == 0)) {//vars with varKey
            ...
            } else if (codeForAnalis.contains(" " + KEY_WORDS[30] + " ")
                    || (codeForAnalis.contains(KEY_WORDS[30] + " ")
                    && codeForAnalis.indexOf(KEY_WORDS[30] + " ") == 0)) {//const with constKey
            ...
            } else if (codeForAnalis.contains(" " + KEY_WORDS[7] + " ")
                    || (codeForAnalis.contains(KEY_WORDS[7] + " ") && codeForAnalis.indexOf(KEY_WORDS[7] + " ") == 0)
                    || (codeForAnalis.contains(" " + KEY_WORDS[8] + " "))
                    || (codeForAnalis.indexOf(KEY_WORDS[8] + " ") == 0)) {// procedures & functions

                boolean isFunction = (codeForAnalis.contains(" " + KEY_WORDS[8] + " "))
                        || (codeForAnalis.indexOf(KEY_WORDS[8] + " ") == 0);

                checkStartOfLine(codeForAnalis, KEY_WORDS[(isFunction) ? 8 : 7] + " ");
                varBufferKey = false;
                constBufferKey = false;
                ...
                for (; i < strings.length; i++) {
                    codeForAnalis = strings[i];
                    if (codeForAnalis.contains(" " + KEY_WORDS[4] + " ") // method body
                            || (codeForAnalis.contains(KEY_WORDS[4] + " ")
                            && codeForAnalis.indexOf(KEY_WORDS[4] + " ") == 0)) {
                        checkStartOfLine(codeForAnalis, KEY_WORDS[4] + " ");
                        procedureInLoad.add(SPECIAL_SYMBOL[0]);
                        int bracketsCount = 0;
                        if (codeForAnalis.contains(" " + KEY_WORDS[5] + " ")
                                || (codeForAnalis.contains(KEY_WORDS[5] + " ") && codeForAnalis.indexOf(KEY_WORDS[5] + " ") == 0)
                                || (codeForAnalis.contains(" " + KEY_WORDS[5]) && codeForAnalis.indexOf(" " + KEY_WORDS[5]) == codeForAnalis.length() - KEY_WORDS[5].length() - 1)) {
                            ...
                            || (codeForAnalis.contains(KEY_WORDS[3] + " ")
                            && codeForAnalis.indexOf(KEY_WORDS[3] + " ") == 0)) { // vars
                        checkStartOfLine(codeForAnalis, KEY_WORDS[3] + " ");
                        procVarBufKey = true;
